#Sex-wise curves of TB variability for selection and control populations over 21 generations of selection
...
df_sel <- read.csv("analysis_by_assay/Generations_Ymaze/sel_F.csv")
df_sel$block <- as.factor(df_sel$block)

df_con <- read.csv("analysis_by_assay/Generations_Ymaze/con_F.csv")
...
ggplot(df_sel, aes(x = gen, y = TB)) +
  ...
  labs(title = "",
       x     = "Generation",
       y     = "Turn-bias Variability (Female)",
       ) +  # legend title for points+
  theme_classic() +
...
ggsave("analysis_by_assay/Generations_Ymaze/selF.pdf",width = 6, height = 4, bg ='transparent')
...
ggsave("analysis_by_assay/Generations_Ymaze/conF.pdf",width = 6, height = 4, bg = 'transparent')

#MALE

df_sel <- read.csv("analysis_by_assay/Generations_Ymaze/sel_M.csv")
df_sel$block <- as.factor(df_sel$block)

df_con <- read.csv("analysis_by_assay/Generations_Ymaze/con_M.csv")
df_con$block <- as.factor(df_con$block)
...
ggsave("analysis_by_assay/Generations_Ymaze/selM.pdf",width = 6, height = 4, bg ='transparent')
...
ggsave("analysis_by_assay/Generations_Ymaze/conM.pdf",width = 6, height = 4, bg = 'transparent')
